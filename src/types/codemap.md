...
3. **State types** (`state.ts`) — runtime state structure, compression block shapes (v1 legacy and v2 scaffold), and persisted state schemas (v1/v2/v3/v4/v5).
...
**Persisted shapes (v1–v5) — supported on session JSONL restore:**
...
| `PersistedDcpStateV2`         | Legacy v2 shape. Carries `blocks: CompressionBlockV2[]` and turn counters; removes `prunedToolIds`/`tokensSaved` (computed at runtime). This feeds deferred-dead Tier-3 scaffolding (`compressionBlocksV2`) and is not a live write shape.                                                                                           |
...
| `PersistedDcpState`           | Union of all persisted shapes (`V1 \| V2 \| V3 \| V4 \| V5 \| Unchanged`).                                                                                                                                                                                                                                                           |
...
| `CompressionBlockV2`       | Inert v2 scaffold loaded only from never-written legacy schema v2. Uses canonical `startSpanKey`/`endSpanKey` instead of timestamps; carries explicit `metadata`. Status is `"active" \| "superseded" \| "decompressed"`. |
...
| Compression   | `schemaVersion: 1 \| 2`, `compressionBlocks: CompressionBlock[]` (active runtime path), `compressionBlocksV2: CompressionBlockV2[]` (inert scaffold/deferred-dead path), `nextBlockId`                                                                                                                                                      |
