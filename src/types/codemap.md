...
| `CompressionLogEntry`      | Persisted deterministic excerpt entry. New rendering consumes only `user_excerpt` / `assistant_excerpt`; legacy tool kinds remain readable for schema compatibility.                     |
...
| `CompressionBlockMetadata` | Hidden exact coverage, modified-file metadata, legacy read/command compatibility fields, and optional aggregate `effectStats` for reads, searches, mutations, commands, and delegations. |
...
| Statistics    | `tokensSaved` (current net from **active** blocks; drops after native compaction deactivates them), `lifetimeTokensSavedRealized` (monotonic accumulator — each deactivated block's `savedTokenEstimate` is added here so displayed savings never regress), `totalPruneCount`, `tokensPruned` (monotonic heuristic-pruning tokens reclaimed — sum of committed candidate `netSaved`; distinct from compress `tokensSaved`; persisted in shared scalars, outside the replay-equivalence contract like `totalPruneCount`). Display total = `tokensSaved + lifetimeTokensSavedRealized`. |
...
| `strategies`       | `pruneCadenceTurns` (bucket boundary for tombstone additions; `1` = per-turn), `minPruneItemSavedTokens` / `minPruneBatchSavedTokens` (net-savings gates before a tombstone busts cache; `0` = off; red-zone bypass), `deduplication` (`enabled`, `protectedTools`), `purgeErrors` (`enabled`, `turns`, `protectedTools`)                     |
