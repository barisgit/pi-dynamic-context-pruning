...
async function handleSweep(
  ...
  if (n > 0) {
    ...
    candidates = lastUserMsgBranchIndex >= 0 ? toolCallIdsSinceLastUser : allToolCallIds;
  }
...
function handleDecompress(
  ...
  if (nArg === undefined) {
    ...
    if (isNaN(id)) {
      ctx.ui.notify(`Invalid block ID: "${nArg}". Usage: /dcp decompress N`, "error");
      return;
    }
...
export function registerCommands(pi: ExtensionAPI, state: DcpState, config: DcpConfig): void {
