...
#### Scenario: Runtime block path remains legacy-compatible

- **WHEN** the layered architecture migration is implemented
- **THEN** the live runtime continues to use the current legacy compression block path with exact metadata enhancements as the primary source of truth
