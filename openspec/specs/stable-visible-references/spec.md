...
#### Scenario: Restored blocks use persisted source-key anchors across session restart

- **WHEN** a session restarts from a coverage-bearing persisted dcp-state entry
- **THEN** restored compression blocks are placed from their persisted `startSourceKey` and `endSourceKey` anchors without replay on resume, so block placement does not depend on reconstructing visible references during the first `context` event
