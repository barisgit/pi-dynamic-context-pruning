...
- `collectStaleResultCandidates()` marks old large **successful** `toolResult`/`bashExecution` outputs (the mass dedup/error-purge never touch) for tombstoning, but only from configured clearable tool-name patterns (`strategies.clearStaleResults.clearTools`, default lowercase read/bash/grep; matching is case-insensitive and supports `*` globs such as `scan_*` or `mcp_*` — tools omitted from the list, including MCP/unknown by default, are protected), after the stale-result retention age (`staleAfterTurns`, default 10 logical turns), above a low size floor (`minResultTokens`), and outside the protected recent tail (`compress.protectRecentTurns` remains an additional hot-tail floor, not the stale retention knob). Like dedup/error-purge it runs **every cadence** and is **not** pressure-gated: it is governed by staleAfterTurns age + protected recent tail + cadence + per-item/batch savings gates, so it can keep context lean _before_ compress is ever needed rather than waking only under pressure. Determinism is preserved exactly like the other two collectors — it must stay disabled in `EQUIVALENCE_CONFIG`, which is why replay produces nothing.
