...
- `collectStaleResultCandidates()` marks old large **successful** `toolResult`/`bashExecution` outputs (the mass dedup/error-purge never touch) for tombstoning, but only from configured clearable tools (`strategies.clearStaleResults.clearTools`, default Read/Bash/Grep — tools omitted from the list, including MCP/unknown, are protected), above a low size floor (`minResultTokens`), outside the protected recent tail. Like dedup/error-purge it runs **every cadence** and is **not** pressure-gated: it is governed only by the shared cadence + per-item/batch savings gates, so it can keep context lean _before_ compress is ever needed rather than waking only under pressure. Determinism is preserved exactly like the other two collectors — it must stay disabled in `EQUIVALENCE_CONFIG`, which is why replay produces nothing.
