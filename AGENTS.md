...
- `collectStaleResultCandidates()` marks old large **successful** `toolResult`/`bashExecution` outputs (the mass dedup/error-purge never touch) for tombstoning, but only from configured clearable tools (`strategies.clearStaleResults.clearTools`, default Read/Bash/Grep — tools omitted from the list, including MCP/unknown, are protected), above a low size floor (`minResultTokens`), outside the protected recent tail. Unlike the other two collectors it is **live-only**: it produces candidates only when the prior-pass effective context is in the cleanup band (`isHeuristicPruneCleanupZone`, reading `state.lastEffectiveContext*` against `compress.minContextPercent` / `minContextTokens`), so replay (which never sets that signal) produces nothing and determinism holds. It must stay disabled in `EQUIVALENCE_CONFIG`.
